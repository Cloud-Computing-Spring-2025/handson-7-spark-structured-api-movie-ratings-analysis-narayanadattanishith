...
def analyze_movie_watching_trends(df):
    ...
    # Step 1: Count movies watched per year
    trend_analysis = df.groupBy("WatchedYear").agg(count("*").alias("Movies Watched"))

    # Step 2: Order results by `WatchedYear`
    trend_analysis = trend_analysis.orderBy(col("WatchedYear"))
...
def main():
    ...
    output_file = "/workspaces/handson-7-spark-structured-api-movie-ratings-analysis-narayanadattanishith/Outputs/movie_watching_trends.csv"
